...
#define REQUIRE_MD5_TYPE  DONOT_DECLARE
...
static void md5_processblock (struct md5 *s, const uint8_t *buf) {
...
static void md5_pad (struct md5 *s) {
  unsigned r = s->len % 64;
...
static void md5_sum (struct md5 *s, uchar md[MD5_DIGEST_LENGTH]) {
  int i;
...
static void md5_update (struct md5 *s, const uchar *m, unsigned long len) {
  const uchar *p = m;
  unsigned r = s->len % 64;
