...
#define REQUIRE_BCRYPT_TYPE   DONOT_DECLARE
...
static int bcrypt_gensalt(int factor, char salt[BCRYPT_HASHSIZE]) {
  ...
  int retval = Random.get.entropy_bytes (input, RANDBYTES);
  if (0 == retval)
    return 1;
...
static char *bcrypt_hash_pw (const char *passwd) {
  char salt[BCRYPT_HASHSIZE];
  char *hash = Alloc (BCRYPT_HASHSIZE + 1);
  if (bcrypt_gensalt (12, salt) == 1)
    goto theerror;

  int retval = bcrypt_hashpw (passwd, salt, hash);
  if (retval)
    goto theerror;
...
static int bcrypt_verify_pw_hash (const char *passwd, const char *hash) {
  char outhash[BCRYPT_HASHSIZE];

  //if (bytelen (hash) isnot BCRYPT_HASHSIZE)
  //  return 0;

  int retval = bcrypt_hashpw (passwd, hash, outhash);
  if (retval) return 0;

  return Cstring.eq_n (outhash, hash, BCRYPT_HASHSIZE);
}
...
public bcrypt_T __init_bcrypt__ (void) {
  ...
  return (bcrypt_T) {
    .self = (bcrypt_self) {
      .hashpw = bcrypt_hash_pw,
      .verify_pw_hash = bcrypt_verify_pw_hash
    }
  };
}

public void __deinit_bcrypt__ (bcrypt_T **thisp) {
  (void) thisp;
  return;
}
