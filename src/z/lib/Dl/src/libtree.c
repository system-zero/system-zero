/* this is libtree from https://github.com/haampie/libtree (see LICENSE within this directory)
 * It is a pity to pollute this code with our own ways to do things, plus will be
 * a pain to sync with upstream, so we'll try to do our intervention without too much noise
 */
...
/* ADDITION S */
static void dl_append_dependency (dl_t *, const char *, int, struct found_t);
static void dl_not_found (size_t, size_t, struct small_vec_u64_t *,  char *, struct libtree_state_t *, int);
...
static int recurse(const char *current_file, size_t depth,
...
#endif
static void print_line(size_t depth, const char *name, const char *color_bold,
                       const char *color_regular, int highlight,
                       struct found_t reason, struct libtree_state_t *s) {
    dl_append_dependency (s->dl, name, depth, reason); return; // ADDITION
    (void) color_bold; (void) color_regular; (void) highlight;
#if 0
...
static int recurse(const char *current_file, size_t depth, struct libtree_state_t *s,
    ...
    s->dl->bits = (curr_bits == BITS64 ? 64 : 32);  s->dl->endian = (is_little_endian ? LITTLE_ENDIAN : BIG_ENDIAN); // ADDITION
    ...
    if (!should_recurse) {
        const char *print_name = soname != MAX_OFFSET_T && !s->path
                               ? s->string_table.arr + soname_buf_offset
                               : current_file;
        const char *bold_color = in_exclude_list ? REGULAR_MAGENTA : REGULAR_BLUE;
        const char *regular_color = in_exclude_list ? REGULAR_MAGENTA : REGULAR_BLUE;
    ...
    const char *bold_color = in_exclude_list ? REGULAR_MAGENTA
                                       : seen_before ? REGULAR_BLUE : BOLD_CYAN;
    const char *regular_color = in_exclude_list
                              ? REGULAR_MAGENTA
    ...
    if (needed_not_found) {
        /* print_error ADDITION */ dl_not_found (depth, needed_not_found, &needed_buf_offsets,
...
static int parse_ld_config_file(struct string_table_t *st, const char *path);
