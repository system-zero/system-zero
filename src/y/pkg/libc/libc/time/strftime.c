// provides: long str_ftime (char *s, long n, const char *f, struct tm *tm)
...
long str_ftime (char *s, long n, const char *f, struct tm *tm) {
  int val;
  char *beg = s;
  char *e = s + n;
  ...
  while (s + 1 < e && *f) {
    ...
    switch (c) {
      ...
      case 'c':
        s += str_ftime (s, e - s, "%b %a %d %k:%M:%S %Z %Y", tm);
        break;
      ...
      case 'r':
        s += str_ftime (s, e - s, "%I:%M:%S %p", tm);
        break;

      case 'R':
        s += str_ftime (s, e - s, "%H:%M", tm);
        break;
      ...
      case 'x':
        s += str_ftime (s, e - s, "%b %a %d", tm);
        break;

      case 'X':
        s += str_ftime (s, e - s, "%k:%M:%S", tm);
        break;
