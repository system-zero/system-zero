...
static void readline_release (readline_t *);
static readline_t *readline_new (void *, term_t *, InputGetch_cb,
      int, int, int, video_t *);
static void readline_insert_with_len (readline_t *, char *, size_t);
static vstring_t *readline_parse_command (readline_t *);
...
static int readline_complete_command (menu_t *menu) {
  readline_t *this = menu->user_data[0];

  if (menu->state & MENU_INIT) {
    menu->state &= ~MENU_INIT;
  } else
    menu_release_list (menu);
  ...
  ifnot (menu->patlen) {
    while (this->commands[i])
      Vstring.add.sort_and_uniq (coms, this->commands[i++]->com);
  } else {
    while (this->commands[i]) {
      ifnot (Cstring.cmp_n (this->commands[i]->com, menu->pat, menu->patlen))
        Vstring.add.sort_and_uniq (coms, this->commands[i]->com);
      i++;
    }
  }
...
static int readline_complete_arg (menu_t *menu) {
  ...
  int com = this->com;
  char *line = this->cur_line->bytes;

  if (this->commands[com]->args is NULL) {
    menu->state |= MENU_QUIT;
    return NOTHING_TODO;
  }
  ...
  ifnot (menu->patlen) {
    while (this->commands[com]->args[i])
      Vstring.add.sort_and_uniq (args, this->commands[com]->args[i++]);
  } else {
    while (this->commands[com]->args[i]) {
      if (Cstring.eq_n (this->commands[com]->args[i], menu->pat, menu->patlen))
        if (NULL is Cstring.bytes_in_str (line, this->commands[com]->args[i]) or
            Cstring.eq (this->commands[com]->args[i], "--fname=") or
            Cstring.eq_n (this->commands[com]->args[i], "--command=", 10))
          Vstring.add.sort_and_uniq (args, this->commands[com]->args[i]);
      i++;
    }
  }
...
getlist:;
  ...
  this->shared_int = joinpath;
  String.replace_with (this->shared_str, dir);
...
finalize:
  ...
  if (this->shared_int) {
    if (this->shared_str->bytes[this->shared_str->num_bytes - 1] is DIR_SEP)
      String.clear_at (this->shared_str, this->shared_str->num_bytes - 1);

    int len = menu->patlen + this->shared_str->num_bytes + 1;
    char tmp[len + 1];
    snprintf (tmp, len + 1, "%s%c%s", this->shared_str->bytes, DIR_SEP, menu->pat);
    String.replace_with (this->shared_str, tmp);
  } else
    String.replace_with (this->shared_str, menu->pat);

  if (Dir.is_directory (this->shared_str->bytes))
...
static int readline_tab_completion (readline_t *this) {
  ifnot (this->line->num_items) return READLINE_OK;

  int retval = READLINE_OK;

redo:;
  Vstring.join_allocated (this->line, "", this->cur_line);
  char *sp = this->cur_line->bytes + this->line->cur_idx;
  char *cur = sp;

  while (sp > this->cur_line->bytes and *(sp - 1) isnot ' ') sp--;
  int fidx = sp - this->cur_line->bytes;
  ...
  if (fidx is 0) {
    ...
    readline_parse_command (this);

    if (this->com isnot READLINE_NO_COMMAND) {
  ...
  menu_t *menu = menu_new (MenuOpts (
    .user_data_first = this,
    .video = this->video,
    .term = this->term,
    .first_row = this->prompt_row - 2,
    .last_row  = this->prompt_row - 2,
    .first_col = 0,
    .prompt_row = this->prompt_row,
    .state = MENU_INIT,
...
//       Msg.send (this, COLOR_WARNING, "--command argument should be enclosed in a pair of '{}' braces");
  ...
  if (type & READLINE_TOK_ARG_FILENAME) {
    ifnot (menu->state & MENU_REDO) {
      String.prepend_with (this->shared_str, "--fname=\"");
      String.append_byte (this->shared_str, '"');
      item = this->shared_str->bytes;
    }
  }

  ifnot (type & READLINE_TOK_ARG_OPTION) {
    DListSetCurrent (this->line, fidx);
  ...
  readline_insert_with_len (this, item, bytelen (item));
...
static readline_t *readline_new (void *user_data, term_t *term, InputGetch_cb getch,
  ...
  this->shared_str = String.new (this->num_cols);
  this->command = String.new (16);
  this->cur_line = String.new (16);
...
static void readline_reset (readline_t *this) {
  ...
  String.clear (this->command);
  String.clear (this->cur_line);
  Vstring.clear (this->line);
  vstring_t *s = Vstring.new_item ();
  s->data = String.new_with_len (" ", 1);
  DListAppendCurrent (this->line, s);
...
static void readline_clear (readline_t *this) {
  ...
  while (row < this->prompt_row)
    video_draw_row_at (this->video, row++, 1);

  if (this->opts & READLINE_OPT_CLEAR_PROMPTLINE_AT_END)
    String.replace_with_len (this->video->rows[this->prompt_row - 1], " ", 1);

  video_draw_row_at (this->video, this->prompt_row, 1);
...
static void readline_write (readline_t *this) {
  ...
  while (this->first_row > orig_first_row)
    video_draw_row_at (this->video, orig_first_row++, 1);
...
theloop:
    switch (this->c) {
      ...
      case ARROW_DOWN_KEY:
        ...
        if (this->history is NULL) goto post_process;
...
insert_char:
        ...
        if ((this->line->cur_idx is this->line->num_items - 1 and ' ' is this->line->current->data->bytes[0]) or
             this->line->cur_idx isnot this->line->num_items - 1) {
...
static vstring_t *readline_parse_command (readline_t *this) {
  ...
  if (it isnot NULL and it->data->bytes[0] is '!') {
    String.append_byte (this->command, '!');
    goto get_command;
  }

  while (it isnot NULL and it->data->bytes[0] isnot ' ') {
    String.append_with_len (this->command, it->data->bytes, it->data->num_bytes);
    it = it->next;
  }
...
get_command:
  ...
  for (i = 0; i < this->num_commands; i++) {
    if (Cstring.eq (this->commands[i]->com, this->command->bytes)) {
      this->com = i;
      break;
    }
  }
