...
#define INTRO(__m__, __mlen__) do {                     \
  ...
  char buf[_len + 1];                                   \
  sys_snprintf (buf, _len + 1, "[%s] %s", nstr, __m__); \
...
#define SUMMARY() do {                               \
  ...
  size_t _len = 1024;                                \
  char buf[_len];                                    \
  int wr = sys_snprintf (buf, _len, "NumTests : [%s]\n", nstr);\
  nstr = int_to_string (&decString, failedTests);    \
  wr += sys_snprintf (buf + wr, _len - wr, "  Failed : [%s]\n", nstr);\
  nstr = int_to_string (&decString, okTests);        \
  wr += sys_snprintf (buf + wr, _len - wr, "  Passed : [%s]\n", nstr);\
  sys_write (1, buf, wr);                            \
} while (0)
